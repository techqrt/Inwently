    ...
    def mapper_value_error(mapped_column_names: dict, columns_required: list):
        for column in columns_required:
            if column not in mapped_column_names.keys():
                raise ValueError(f'{column} not a proper column names')
        ...
        def exceptions(*args, **kwargs):
            try:
                ...
                if 'foreign key constraint' in str(e):
                    return Response(status=status.HTTP_400_BAD_REQUEST,
                                    data=Utils.error_response_data(error=[self.foreign_key_error],
                                                                   message='Foreign Key Error ' + Utils.env_exception_handler(
                                                                       message=str(e))))
                return Response(status=status.HTTP_400_BAD_REQUEST,
                                data=Utils.error_response_data(error=[self.db_error],
                                                               message='Exception Error ' + Utils.env_exception_handler(
                                                                   message=str(e))))
            return fun
