...
class CategoryEsQuery:
    @staticmethod
    def search_pattern_start_with_query(organisation_id: int, request_keys: str,
                                        response_keys=None) -> list:
        """
            response_keys = ['name', 'category_code']
        """
        ...
        response = es.general_search(index_name='category', search_keys=request_keys)
        data = []
        for hit in response:
