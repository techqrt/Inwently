...
from biller_apps.purchase.models import PurchaseBills
...
class ReturnPurchase(models.Model):
    ...
    purchase = models.ForeignKey(PurchaseBills, on_delete=models.CASCADE, related_name='return_purchases')
    ...
    def get(return_code: str, organisation_name: str) -> dict:
        return ReturnPurchase.objects.filter(return_code=return_code, organisation_id__company_name=organisation_name).values(
    ...
    def get_all(organisation_name: str,params:GetAll) -> list:
        ...
        return ReturnPurchase.objects.filter(filters).values(
            'return_code', 'purchase__purchase_bill_id', 'supplier__name', 'created_date_time', 'total_price'
        ).order_by(params.ordering)

    @staticmethod
    def update(return_id: int, return_reason: str, quantity: float, tax: float, total_price: float) -> int:
        return_purchase = ReturnPurchase.objects.get(return_id=return_id)
        return_purchase.return_reason = return_reason
        return_purchase.quantity = quantity
        return_purchase.tax = tax
        return_purchase.total_price = total_price
        return_purchase.save()
        return return_purchase.return_id

    @staticmethod
    def calculate_total_price(quantity: float, tax: float) -> float:
        return quantity + tax

    @staticmethod
    def remove(return_id: int):
        ReturnPurchase.objects.get(return_id=return_id).delete()
